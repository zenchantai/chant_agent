...
打开分时图先显示缓存，再获取腾讯行情源最新的一分钟数据。按上海时区与交易日历，在 09:30–11:30、13:00–15:00 每 15 秒刷新；午休、收盘后延迟 15 秒补取一次，然后暂停至下一交易时段。休市时打开或从隐藏页面返回会补取一次，切换其他周期或隐藏页面时停止轮询。

刷新只更新当前股票的分时行情，不执行全周期同步或结构重算；副图、缩放和未保存的绘图不因自动刷新重置。失败时保留旧图并显示提示，非今日数据明确标注日期；交易日历缺失时显示“日历未知”，每 60 秒尝试恢复日历。15 秒是请求频率，不代表数据源提供逐笔或秒级行情。

接口 `GET /api/chart-data/{symbol}?timeframe=1&refresh=true` 主动获取最新数据；省略 `refresh` 时只读缓存。`refresh=true` 不允许搭配其他周期或 `before`。响应中的 `intraday_refresh` 包含市场阶段、服务器时间、下次阶段切换时间、数据日期、最新数据时间、最近获取成功时间和错误提示。昨收无法确认或成交额不可用时显示 `--`。
