export type Bar={trade_date:string;open:number;high:number;low:number;close:number;volume:number;amount:number;is_forming?:boolean;status?:"provisional"|"confirmed"};
...
export type IntradayRefresh={server_time:string;phase:"auction"|"trading"|"lunch"|"closed"|"unknown";market_status:string;next_transition_at:string|null;next_bar_finalize_at?:string|null;data_date:string|null;latest_data_at:string|null;last_success_at:string|null;result:"cached"|"success"|"failed"|"stale";error:string|null;calendar_error?:string|null;is_today:boolean};
...
export interface Center extends StructureBase {kind:"center";family_id:string;revision_no:number;previous_revision_id?:string|null;zd:number;zg:number;fixed_zd:number;fixed_zg:number;dd:number;gg:number;fluctuation_dd:number;fluctuation_gg:number;core_start_date?:string;core_end_date?:string;entry_direction?:string|null;core_formation_pattern?:string;departure_direction?:string|null;owner_movement_id?:string|null;entry_component_id?:string|null;departure_component_id?:string|null;retest_component_id?:string|null;entry_unit_ids:string[];core_unit_ids:string[];extension_unit_ids:string[];peripheral_unit_ids:string[];departure_unit_ids:string[];retest_unit_ids:string[];owned_unit_ids:string[];context_unit_ids:string[];child_center_ids:string[];child_movement_ids:string[];formation_modes:string[];recursive_eligible:boolean;absorbed_into_family_id?:string;}
export interface Movement extends StructureBase {kind:"movement";family_id:string;revision_no:number;previous_revision_id?:string|null;classification:"consolidation"|"trend"|null;start_price:number;end_price:number;point_date?:string|null;start_point_id?:string|null;end_point_id?:string|null;source_unit_ids:string[];center_family_ids:string[];center_revision_ids:string[];center_levels:number[];child_movement_ids:string[];price_envelope_low:number;price_envelope_high:number;candidate_extreme_date?:string|null;candidate_extreme_price?:number|null;recursive_eligible:boolean;termination_reason:string;undetermined_reason?:string|null;}
export interface StructuralPoint extends StructureBase {kind:"structural_point";family_id:string;revision_no:number;point_type:string;point_date:string;point_price:number;source_unit_id:string;source_component_ids:string[];center_family_id:string;center_revision_id:string;movement_family_id?:string|null;parent_point_id?:string|null;invalidated_reason?:string|null;}
export type StructureEntity=Pen|Component|Center|Movement|StructuralPoint;
export type CenterDisplayReference={revision_id:string;display_role:"active"|"constituent";parent_revision_ids:string[]};
export interface Center {display_role?:"active"|"constituent";parent_revision_ids?:string[];temporary_evidence?:boolean;selected_evidence?:boolean;unit_kind?:"pen"|"movement"|"center_revision";z_unit_ids:string[];z_direction?:"up"|"down"|null;formed_at?:string;promotion_confirmed_at?:string|null;connection_component_ids:string[];overlap_witness_unit_ids:string[];missing_evidence:string[];context_low?:number;context_high?:number;}
...
export type CalculationProfile = "full" | "pen_centers_only";
...
export type ChartApiResponse={overlays?:ChartOverlays;meta:ChartMeta;market:{symbol:string;timeframe:string;adjustflag:string;bars:Bar[];previous_close?:number|null;quote?:Quote|null;forming_bar?:{trade_date:string;is_forming:boolean;status:"provisional"|"confirmed";finalize_at?:string}|null;intraday_refresh?:IntradayRefresh};structure:{display_centers?:CenterDisplayReference[];display_center_levels?:number[];pens:Pen[];components:Component[];centers:Center[];center_revisions:Center[];movements:Movement[];movement_revisions:Movement[];points:StructuralPoint[];point_revisions:StructuralPoint[];relations:Record<string,unknown>[];issues:Record<string,unknown>[];levels:number[];unassigned_by_level:Record<string,string[]>;pen_diagnostics?:PenDiagnostic[]};indicators:{macd:MacdPoint[];ma?:MaPoint[];boll?:BollPoint[]};drawings:{items:Drawing[];version:string};pagination:{has_more:boolean;next_before?:string|null}};

export type ChartData={calculation_profile?:CalculationProfile;overlays?:ChartOverlays;display_centers?:CenterDisplayReference[];display_center_levels?:number[];symbol:string;timeframe:string;adjustflag:string;previous_close?:number|null;quote?:Quote|null;intraday_refresh?:IntradayRefresh;forming_bar?:{trade_date:string;is_forming:boolean;status:"provisional"|"confirmed";finalize_at?:string}|null;bars:Bar[];pens:Pen[];pen_diagnostics?:PenDiagnostic[];centers:Center[];center_revisions:Center[];movements:Movement[];movement_revisions:Movement[];components:Component[];points:StructuralPoint[];point_revisions:StructuralPoint[];relations:Record<string,unknown>[];issues:Record<string,unknown>[];levels:number[];unassigned_by_level:Record<string,string[]>;center_levels:number[];movement_levels:number[];drawings:Drawing[];drawings_version:string;indicators:{macd:MacdPoint[];ma?:MaPoint[];boll?:BollPoint[]};has_more:boolean;next_before?:string;available:boolean;stale_reason?:string;definition_version:string;calculator_fingerprint:string;structure_version:string;market_version?:string;run_id?:number;active_structure_level:number;max_available_center_level:number;coverage?:Coverage;structure_preview?:boolean;structure_persisted?:boolean};
