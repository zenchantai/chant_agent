...
import { buildCenterAreas, buildPenSeries, buildPriceSeries, normalizeChartData } from "./chartBuilders";
...
const center = (id: string, level: number, active: boolean): Center => ({
  ...
  entry_unit_ids: [], core_unit_ids: [], z_unit_ids: [], child_center_ids: [], child_movement_ids: [], source_pen_ids: [], formation_modes: [],
...
const fixture = () => {
  ...
  const point = { id: "point", kind: "structural_point", center_revision_id: old.id, source_component_ids: ["departure", "retest"] };
  const data = { centers: [parent], center_revisions: [old, child, right, parent], points: [point], pens: [], movements: [], movement_revisions: [], components: [], levels: [2],
  ...
  return { data, old, child, right, parent, point };
};
...
describe("expanded center display", () => {
  ...
  it("selects exact point revision, replaces representative, and restores after clearing", () => {
    const { data, old, child } = fixture();
    expect(selectedCenterEvidence(data, "point")?.id).toBe(old.id);
    const selected = centersForDisplay(data, "point");
  ...
  it("reveals hidden exact evidence without changing switches and does not fallback on missing references", () => {
    ...
    const visible = centersForDisplay(data, "point", false, levels);
    ...
    data.points[0].center_revision_id = "missing";
    expect(selectedCenterEvidence(data, "point")).toBeUndefined();
    expect(centersForDisplay(data, "point", false, levels)).toEqual([]);
  });
  it("draws parents before children, uses solid constituent outlines, and renders hidden selected evidence", () => {
    ...
    const context = { data, selectedStructureId: "point", visible: { centers: false } };
  ...
  it("links an earlier revision to the expansion parent without replacing its point evidence", () => {
  ...
  it("highlights point-specific departure and retest source pens", () => {
    const { data } = fixture();
    data.pens = ["depart-pen", "retest-pen"].map(id => ({ id, kind: "pen", status: "confirmed", start_date: "2026-01-01", end_date: "2026-01-03", start_price: 10, end_price: 12 } as any));
    data.components = ["departure", "retest"].map((role, index) => ({ id: role, role, source_unit_ids: [data.pens[index].id] } as any));
    const series = buildPenSeries({ data, selectedStructureId: "point" }, ["2026-01-01", "2026-01-03"]);
    expect(series.map(item => item.lineStyle.color)).toEqual(["#dc2626", "#16a34a"]);
  });
});
