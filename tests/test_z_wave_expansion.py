...
def test_owned_z_envelope_distinguishes_newborn_expansion_and_touch(middle, relation, levels, mirror):
    prices = [1, 10, 6, 15, 8, 20, middle, 25, 18, 30, 26, 35]
    if mirror:
    ...
    assert children[1]["z_unit_ids"] == ["p5", "p7"]
    assert children[0]["z_direction"] == ("up" if mirror else "down")
    expected = [(6, 15), (middle, 25)]
    if mirror:
...
def test_expansion_audit_rejects_missing_witness():
    ...
    event["evidence"]["overlap_witness_unit_ids"] = []
    assert any("witness" in e for e in validate_structure({"pens": pens, **result}))
...
def test_overlap_without_complete_proof_keeps_children_active(missing):
    units = atomic_pen_units(pens_from_prices([1, 10, 6, 15, 8, 20, 12, 25, 18, 30, 26, 35]))
